    ...
    def forward(self, x):
        ...
        for i in range(len(self.decoder_blocks)):
            ...
            if target_size is None:
                target_size = xi.shape[2:]  # (H, W) of the first upsampled feature
...
class PointPillars(nn.Module):
    def __init__(self,
                 ...
                 point_cloud_range=[-70.4, -40.0, -3, 70.4, 40.0, 1], ### FULL 360 RANGE !!!
                 max_num_points=32,
                 max_voxels=(32000, 80000)): ### DOUBLES max_voxels
        super().__init__()
        ...
        # anchors
        ranges = [[-70.4, -40.0, -1.78, 70.4, 40.0, -1.78],
                  [-70.4, -40.0, -1.78, 70.4, 40.0, -1.78],
                  [-70.4, -40.0, -0.6, 70.4, 40.0, -0.6]]
        sizes = [[2.05345745, 4.85030859, 1.79081998], # [w, l, h]
                 [3.18005558, 8.87054562, 3.88051096],
                 [0.82310085, 2.10169815, 1.59044227]]
        rotations=[0, 1.57]
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None):
        ...
        ### after training, bbox_pred should match anchor, which is x, y, z, w, l, h, yaw
        
        if mode == 'train':
            batched_gt_bboxes = [gt_bboxes[:, [0, 1, 2, 4, 5, 3, 6]] for gt_bboxes in batched_gt_bboxes] # change from hwl to wlh to match anchor box
